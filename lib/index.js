...
function launch(config){
  createHttpServer(command.norm(config), setupServer);
}

function setupServer(httpServer, options){
  ...
  files.awaitFile = fsWatcher.awaitFile;
  files.startWatch = fsWatcher.startWatch;
  ...
  if (options.sync)
  {
    ...
    require('./modules/file-sync')(wsServer, options);
    (options.rempl || require('rempl/server'))(wsServer, httpServer, options);
  }
}
